...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  fileprivate func setupStatusItem() {
    if let button = statusItem.button {
      button.title = TaskBLL.shared().remainTimeText.value
      TaskBLL.shared().remainTimeText.signal.observeValues({ [weak button] (timeStr) -> () in
    ...
    menu.addItem(NSMenuItem(title: "Quit TomatoThings", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
    
    statusItem.menu = menu
  }
    
    // MARK: Menu actions
    func openTomatoThingsAction(_ sender: AnyObject) {
  ...
  lazy var applicationDocumentsDirectory: URL = {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "top.ioslearn.TomatoThings" in the user's Application Support directory.
    let urls = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)
    let appSupportURL = urls[urls.count - 1]
    return appSupportURL.appendingPathComponent("top.ioslearn.TomatoThings")
  }()
  
  lazy var managedObjectModel: NSManagedObjectModel = {
    // The managed object model for the application. This property is not optional. It is a fatal error for the application not to be able to find and load its model.
    let modelURL = Bundle.main.url(forResource: "TomatoThings", withExtension: "momd")!
